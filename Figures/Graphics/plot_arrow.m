...
if max(size(p0))==3
    if max(size(p1))==3
        ...
        z1 = p1(3);
        if(redraw)
            h(1) = plot3([x0;x1],[y0;y1],[z0;z1]);   % Draw a line between p0 and p1
        else
            set(h(1), 'XData', [x0;x1], 'YData', [y0;y1],'ZData',[z0;z1]);
        end
        p = p1-p0;
        ...
        hold on
        if(redraw)
            h(2) = plot3(hu(:),hv(:),hw(:));  % Plot arrow head
        else
            set(h(2), 'XData', hu(:), 'YData', hv(:),'ZData',hw(:));
        end
        hold off
    else
...
elseif max(size(p0))==2
    if max(size(p1))==2
        ...
        y1 = p1(2);
        if(redraw)
            h(1) = plot([x0;x1],[y0;y1]);   % Draw a line between p0 and p1
        else
            set(h(1), 'XData', [x0;x1], 'YData', [y0;y1]);
        end
        p = p1-p0;
        alpha = 0.1;  % Size of arrow head relative to the length of the vector
        beta = 0.1;  % Width of the base of the arrow head relative to the length
        ...
        hold on
        if(redraw)
            h(2) = plot(hu(:),hv(:));  % Plot arrow head
        else
            set(h(2), 'XData', hu(:), 'YData', hv(:));
        end
        hold off
    else
        error('p0 and p1 must have the same dimension')
